...
const updateProductInCart = catchAsync(async (req, res) => {
  ...
  const cartData = await cartService.updateProductInCart(
    req.user,
    req.body.productId,
    req.body.quantity
  )
  if(req.body.quantity == 0){
    console.log(cartData, 'cart data from controller')
    return res.status(httpStatus.NO_CONTENT).send(cartData)
  }
